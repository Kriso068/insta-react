...
const ChatBox = () => {
    ...
    const [messages, setMessages] = useState([]);
    const [senderUserProfile, setSenderUserProfile] = useState(null);
    const [receiverUid, setReceiverUid] = useState(null); // Store receiver UID
    const scroll = useRef();
    const [editingMessage, setEditingMessage] = useState(null);
    const [editText, setEditText] = useState("");
    ...
    useEffect(() => {
        ...
        const q = query(
            collection(firestore, "privateMessages"),
            where("users", "in", [[authUser.uid, senderUserProfile.uid], [senderUserProfile.uid, authUser.uid]]), // ✅ Valid Firestore Query
            orderBy("createdAt", "desc")
        );
        ...
        const unsubscribe = onSnapshot(q, (querySnapshot) => {
            ...
            if (fetchedMessages.length > 0) {
                ...
                setReceiverUid(firstMessage.receiverUid);
            }
    ...
    // 🔹 Redirect to home (`/`) if authUser is NOT the receiver
    if (receiverUid && authUser?.uid !== receiverUid) {
        return <Navigate to="/" />;
    }
